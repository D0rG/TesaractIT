...
public class Portal : MonoBehaviour {
    ...
    RenderTexture viewTexture;
     Camera portalCam; 
    public Camera playerCam;// change private
    ...
    void HandleTravellers () {
        ...
        for (int i = 0; i < trackedTravellers.Count; i++) {
            ...
            if ((portalSide != portalSideOld)/*&&(traveller.readyToTeleport)*/) {
                ...
                /*if  (traveller.readyToTeleport)*/ traveller.previousOffsetFromPortal = offsetFromPortal;
            }
        }
    }
    ...
    void CreateViewTexture () {
        if (viewTexture == null || viewTexture.width != playerCam.pixelWidth || viewTexture.height != playerCam.pixelHeight) {
            if (viewTexture != null) {
                viewTexture.Release ();
            }
            viewTexture = new RenderTexture (playerCam.pixelWidth, playerCam.pixelHeight, 0);
            // Render the view from the portal camera to the view texture
            portalCam.targetTexture = viewTexture;
            // Display the view texture on the screen of the linked portal
            linkedPortal.screen.material.SetTexture ("_MainTex", viewTexture);
            //linkedPortal.screen.material.SetFloat ("_Add", Add);
        }
    }
